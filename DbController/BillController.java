...
public class BillController {
    ...
    public Bill getBill(String id) throws SQLException{
        Connection conn=getConnection();
        Statement statement=conn.createStatement();
        String query = "select * from ambulance where bill_no='"+id+"'";
        ResultSet rs=statement.executeQuery(query);
        if(rs.next()){
                                                                                                                                                  //String billNo, String patientId, double doctorCharge, double medicineCharge, double roomCharge, double operationCharge, int noOfDays, double healthCard, double labCharge, double advancePay, double totalBill);
